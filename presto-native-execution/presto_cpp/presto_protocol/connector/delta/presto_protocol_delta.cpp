...
void to_json(json& j, const DeltaColumn& p) {
  j = json::object();
  to_json_key(j, "id", p.id, "DeltaColumn", "Long", "id");
  to_json_key(
      j,
      "physicalName",
      p.physicalName,
      ...
      "physicalName");
  to_json_key(
      j, "logicalName", p.logicalName, "DeltaColumn", "String", "logicalName");
...
void from_json(const json& j, DeltaColumn& p) {
  from_json_key(j, "id", p.id, "DeltaColumn", "Long", "id");
  from_json_key(
      j,
      "physicalName",
      p.physicalName,
...
void to_json(json& j, const DeltaColumnHandle& column) {
  j = json::object();
  j["@type"] = "hive-delta";
  to_json_key(j, "id", column.id);
  to_json_key(j, "physicalName", column.physicalName);
  to_json_key(j, "columnName", column.logicalName);
  to_json_key(j, "dataType", column.dataType);
  to_json_key(j, "columnType", column.columnType);
  to_json_key(j, "subfield", column.subfield);
}

void from_json(const json& j, DeltaColumnHandle& column) {
  column._type = j.at("@type");
  from_json_key(j, "id", column.id);
  from_json_key(j, "physicalName", column.physicalName);
  from_json_key(j, "columnName", column.logicalName);
  from_json_key(j, "dataType", column.dataType);
  from_json_key(j, "columnType", column.columnType);
  from_json_key(j, "subfield", column.subfield);
}
...
void to_json(json& j, const DeltaSplit& split) {
  j = json::object();
  j["@type"] = "hive-delta";
  to_json_key(j, "connectorId", split.connectorId);
  to_json_key(j, "schemaName", split.schemaName);
  to_json_key(j, "tableName", split.tableName);
  to_json_key(j, "tableLocation", split.tableLocation);
  to_json_key(j, "filePath", split.filePath);
  to_json_key(j, "start", split.start);
  to_json_key(j, "length", split.length);
  to_json_key(j, "fileSize", split.fileSize);
  to_json_key(j, "partitionValues", split.partitionValues);
  to_json_key(j, "nullPartitionKeys", split.nullPartitionKeys);
}

void from_json(const json& j, DeltaSplit& split) {
  split._type = j.at("@type");
  from_json_key(j, "connectorId", split.connectorId);
  from_json_key(j, "schemaName", split.schemaName);
  from_json_key(j, "tableName", split.tableName);
  from_json_key(j, "tableLocation", split.tableLocation);
  from_json_key(j, "filePath", split.filePath);
  from_json_key(j, "start", split.start);
  from_json_key(j, "length", split.length);
  from_json_key(j, "fileSize", split.fileSize);
  from_json_key(j, "partitionValues", split.partitionValues);
  from_json_key(j, "nullPartitionKeys", split.nullPartitionKeys);
}
...
void to_json(json& j, const DeltaTransactionHandle& handle) {
  j = json::array();
  j.push_back(handle._type);
  j.push_back(handle.instance);
}

void from_json(const json& j, DeltaTransactionHandle& handle) {
  j.at(0).get_to(handle._type);
  j.at(1).get_to(handle.instance);
}
